...
def calculate_eco_score(product_data):
    ...
    # 1. Category-based scoring (some categories are inherently more eco-friendly)
    category_scores = {
        'electronics': 30,
        'fashion': 40,
        'home': 60,
    ...
    score += category_scores.get(product_data.get('category', 'other'), 0) - 50
    
    # 2. Condition-based scoring (better condition = more eco-friendly)
    ...
    # 3. Price ratio (bigger discount from original price = more eco-friendly)
    try:
        original_price = float(product_data.get('original_price', 0))
        selling_price = float(product_data.get('price', 0))
        if original_price > 0 and selling_price > 0:
            discount_ratio = (original_price - selling_price) / original_price
            score += min(20, discount_ratio * 100)  # Up to 20 points for big discounts
    except:
        pass
    
    # 4. Age factor (newer items might be less eco-friendly than older ones being reused)
    # This would need a manufactured_date field which we don't have currently
    
    # 5. Seller reputation (not implemented here but could be added)
    
    # Ensure score is within 0-100 range
    return max(0, min(100, score))

# Update your add_product and edit_product routes to calculate eco score
...
def add_product():
    ...
    if request.method == 'POST':
        ...
        # In add_product route, after getting form data:
        ...
        # Then include eco_score in your INSERT statement
        cursor.execute(
            'INSERT INTO products (user_id, title, description, category, price, image_url, location, is_active, eco_score) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)',
            (session['id'], title, description, category, price, image_url, location, 1, eco_score)
        )
